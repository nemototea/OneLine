...
import net.chasmine.oneline.data.git.MigrationOption
...
/**
 * 設定画面のViewModel（共通コード）
 *
 * @param settingsManager 設定マネージャー
 * @param gitRepositoryService Gitリポジトリサービス
 */
class SettingsViewModel(
    ...
    fun saveSettings(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                // リポジトリを初期化
                val result = gitRepositoryService.initRepository(repoUrl, username, token)
    ...
    fun validateRepository(repoUrl: String, username: String, token: String) {
        viewModelScope.launch {
            ...
            try {
                // 安全な検証方式を使用（クローンなし）
                val validationResult = gitRepositoryService.validateRepositorySafely(repoUrl, username, token)
                ...
                _uiState.value = UiState.ValidationResult(
                    result = ValidationResult.VALIDATION_FAILED,
        ...
        viewModelScope.launch {
            ...
            try {
                val option = when (migrationOption) {
                    "MIGRATE_DATA" -> MigrationOption.MIGRATE_DATA
                    "DISCARD_AND_SWITCH" -> MigrationOption.DISCARD_AND_SWITCH
                    else -> MigrationOption.DISCARD_AND_SWITCH
                }
                ...
                // リポジトリ移行を実行
                val result = gitRepositoryService.migrateToNewRepository(repoUrl, username, token, option)
    ...
    sealed class UiState {
        ...
        data class ValidationResult(val result: net.chasmine.oneline.data.git.ValidationResult, val message: String) : UiState()
